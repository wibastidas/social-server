...
function getReceiverMessage(req, res){
    ...
    Message.find({receiver: userId}).sort('-created_at').populate('emmiter', 'name surname _id nick image').paginate(page, itemsPerPage, (err, messages, total) => {
        if(err) return res.status(500).send({message: 'Error en la petición'});
...
function getEmmitMessages(req, res){
    ...
    Message.find({emmiter: userId}).sort('-created_at').populate('emmiter receiver', 'name surname _id nick image').paginate(page, itemsPerPage, (err, messages, total) => {
        if(err) return res.status(500).send({message: 'Error en la petición'});
